...
- Name: homepage.spec.ts >> Homepage >> should have functional search
- Location: tests\homepage.spec.ts:17:7

# Error details

```
Error: locator.isVisible: Error: strict mode violation: locator('input[placeholder*="search" i]') resolved to 2 elements:
    1) <input name="q" value="" type="search" aria-label="Search products" placeholder="Search products" class="w-full bg-transparent text-sm outline-none placeholder:text-slate-400 dark:text-slate-100 dark:placeholder:text-slate-500"/> aka getByRole('searchbox', { name: 'Search products' })
    2) <input name="q" value="" type="search" aria-label="Search products" placeholder="Search products" class="w-full bg-transparent text-sm outline-none placeholder:text-slate-400 dark:text-slate-100 dark:placeholder:text-slate-500"/> aka getByPlaceholder('Search products').nth(1)

Call log:
    - checking visibility of locator('input[placeholder*="search" i]')
